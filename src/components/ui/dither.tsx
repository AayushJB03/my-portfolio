...
import { useRef, useEffect, forwardRef, type Ref } from "react"
...
void main() {
  ...
  float wipeEdge = transitionProgress * 1.4 - 0.2;
  float wipe = smoothstep(wipeEdge - 0.15, wipeEdge + 0.15, uv.x);
  vec3 activeWaveColor = mix(waveColor, prevWaveColor, wipe);
  
  vec2 uvCentered = uv - 0.5;
  uvCentered.x *= resolution.x / resolution.y;
  float f = pattern(uvCentered);
  vec3 col = mix(baseColor, activeWaveColor, clamp(f, 0.0, 1.0));
...
class RetroEffectImpl extends Effect {
  ...
  set ditherBias(v: number) {
    this.uniforms.get("ditherBias")!.value = v
  }
...
const RetroEffect = forwardRef(function RetroEffect(
  { colorNum, pixelSize, ditherBias }: RetroEffectProps,
  ref: Ref<Effect>
) {
  return (
    <WrappedRetro
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    prevWaveColor: new THREE.Uniform(new THREE.Color(...waveColor)),
    transitionProgress: new THREE.Uniform(1.0),
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevWaveColor.current.every((v, i) => v === waveColor[i])) {
      ;(u.prevWaveColor.value as THREE.Color).set(
        prevWaveColor.current[0],
        prevWaveColor.current[1],
        prevWaveColor.current[2]
      )
    ...
    if (transitionStartTime.current >= 0) {
      const elapsed = clock.getElapsedTime() - transitionStartTime.current
      const t = Math.min(elapsed / TRANSITION_DURATION, 1)
      u.transitionProgress.value = t
      if (t >= 1) {
        transitionStartTime.current = -1
      }
    }

    if (!prevBaseColor.current.every((v, i) => v === baseColor[i])) {
      ;(u.baseColor.value as THREE.Color).set(
        baseColor[0],
        baseColor[1],
        baseColor[2]
      )
  ...
  return (
    <>
      <color attach="background" args={[baseColor[0], baseColor[1], baseColor[2]]} />
