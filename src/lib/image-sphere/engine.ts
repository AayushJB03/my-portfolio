...
export class ImageSphere {
  ...
  constructor(
    ...
    this.host = host
    this.focusFill = opts.focusFill ?? DEFAULT_FOCUS_FILL
    this.planeSize = opts.planeSize ?? DEFAULT_PLANE_SIZE
  ...
  private loadPlanes(urls: string[]) {
    ...
    urls.forEach((url) => {
      loader.load(url, (tex) => {
        ...
        const r = this.radius + (Math.random() - 0.5) * 64
  ...
  private bindEvents() {
    ...
    const onUp = (event: PointerEvent) => {
      ...
      if (moved <= CLICK_SLOP && this.running) {
        ...
        localMouse(event.clientX, event.clientY)
        const hit = this.pick()
        this.focused = this.focused ? (hit === this.focused ? null : hit) : hit
      }
    }
    const onLeave = () => {
      release()
    ...
    const onKey = (event: KeyboardEvent) => {
      if (event.key === "Escape" && this.focused) this.focused = null
    }
  ...
  private loop = () => {
    ...
    const viewH =
      2 * FOCUS_DISTANCE * Math.tan((this.camera.fov * Math.PI) / 360)
    const focusScale = (viewH * this.focusFill) / this.planeSize
    ...
    for (const plane of this.planes) {
      ...
      const f =
        plane.userData.focus + (focusTarget - plane.userData.focus) * FOCUS_EASE
      ...
      const wantOpacity = anyFocused ? BACKDROP_DIM + (1 - BACKDROP_DIM) * f : 1
      const o =
        plane.userData.opacity +
        (wantOpacity - plane.userData.opacity) * OPACITY_EASE
