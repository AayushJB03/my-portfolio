...
export async function generateMetadata({
  ...
  const project = await getProjectByHref(href)
  ...
  return generateMetaData(project.title, project.description, {
    path: project.href,
  })
}
...
const ProjectPage = async ({ params }: ProjectPageProps) => {
  ...
  if (!project) {
    return (
      <ContainerWrapper>
        <Separator />
        <BackButton title="Projects" href="/projects" />
        <Separator />
  ...
  const structuredData = {
    ...
    url: `${SITE_INFO.url}${project.href}`,
    image: project.image,
    ...
    sameAs: [project.githubLink, project.liveLink].filter(Boolean),
  }
    ...
    <ContainerWrapper>
      ...
      <div className="p-2 md:p-4">
        <Card className={cn("group p-2")}>
          <div
            className={cn(
              "bg-muted border-edge relative h-[200px] w-full overflow-hidden rounded-[6px] border select-none sm:h-[400px] md:h-[500px]",
          ...
          >
            <TextScramble
              as="h1"
              className="text-muted-foreground absolute top-3 left-2 text-xs font-medium transition-all duration-300 group-hover:left-1/2 group-hover:-translate-x-1/2 sm:text-base"
            >
              {project.title}
            </TextScramble>
              ...
              <div className="h-full w-full overflow-hidden rounded-t-[4px]">
                <div className="hidden dark:block">
                  <Image
                    ...
                    sizes="(min-width: 768px) 52rem, calc(100vw - 3rem)"
        ...
        {links.map((item) => {
          return (
            <a
              ...
              href={item.href}
              target="_blank"
              className="border-edge group relative flex w-1/2 cursor-pointer flex-row rounded-sm border-[1px] border-dotted"
            >
              <div
                key={item.name}
              ...
              >
                ...
                <div className="border-edge h-[80%] border-r-[1px] border-dotted" />
